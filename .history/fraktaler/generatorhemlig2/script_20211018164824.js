...
var fraktal=(function (){
	...
	modul.al =(function(){
		...
		var prepare = function prepare(){
			...
			//now test for the highest/lowest the line gets after skew correction is applied. if it is too much i change the way the shape updates.
	...
	var makeThumb = function(spec){
		...
		outward.hover = function (){
			...
			ctx.fillStyle="#ffffff";
			if (updates[this.id]){
				updates[this.id]((outward.step+1)%outward.steps);
			}
		};
		...
		updates.segments = function (step){
			function oneline(){
