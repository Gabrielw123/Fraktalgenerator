...
var fract=(function (){
	...
	module.init = function (){
		...
		ctx = c.getContext("2d");
		fract.vinkel = makeSlider({steps:360/5, step:0/5, id:"vinkel"});
		fract.skewvinkel = makeSlider({steps:180/5, step:90/5, id:"skewvinkel"});
		fract.poly = makeThumb({steps:5, step:0, id:"poly"});
		...
		fract.vinkel.fix = function () {
			...
			var skew=fract.skewvinkel.step-fract.skewvinkel.steps/2;
			fract.skewvinkel.steps= 180/5*segments;
			if (fract.arms.step){
				fract.skewvinkel.steps*=2;	
				if (segments%2===0){
					fract.vinkel.steps*=2;
				}
			}
			...
			if (Math.abs(skew)>fract.skewvinkel.steps/2){
				fract.skewvinkel.step=(fract.skewvinkel.step+fract.skewvinkel.steps/2)%fract.skewvinkel.steps;
			}
			else{
				fract.skewvinkel.step=fract.skewvinkel.steps/2+skew;
			}
			fract.vinkel.update(this.step,0,1);
			fract.skewvinkel.update(fract.skewvinkel.step,0,1);
		}
		
		fract.skewvinkel.updateLabel =function(){
			this.label.innerHTML=Math.round((this.step-this.steps/2)*5);
		}
		fract.skewvinkel.updateLabel();
	...
	module.fixsize = function (){
		...
		ctx.lineWidth=0.5;
		fract.vinkel.resize();
		fract.skewvinkel.resize();
		clear();
		fract.al.render();
	}
	//Tabort allt
	module.reset = function (){
		fract.vinkel.update(0);
		fract.depth.update(0);
		fract.segments.update(0);
		fract.arms.update(0);
		fract.skewvinkel.update(90/5);
	...
	module.animate = (function (){
		...
		module.toggle = function (){
			...
			if (this.on){
				...
				if (!fract.vinkel.locked || !fract.skewvinkel.locked){
					this.on=true;
					knapp.className=("knappon");
					module.loop();
				}
		...
		module.loop = function (){
			if (module.on){
				...
				fract.vinkel.nudge(1/2);
				fract.skewvinkel.nudge(1/5);
				if (fract.auto.on){
					fract.auto.frame();
				}
				fract.al.render();
	...
	module.auto = (function (){
		...
		var symmetrystart = function(){
			...
			pre=128;
			fract.vinkel.unlock();
			fract.skewvinkel.lock();
			fract.random();
			fract.skewvinkel.update(fract.skewvinkel.steps/2);
		}
		return module;
	}());
	...
	module.hash =(function(){
		...
		module.save=function(){
			var ruleid = "#" +
			(('000'+(Math.round(fract.vinkel.step*5))).substr(-4)) + "," +
			('000'+(Math.round(fract.skewvinkel.step*5))).substr(-4) + "," +
			(fract.poly.step+1) + "," +
			...
			if (window.location.hash != 0){
				if (window.location.hash !== module.current){
					...
					if (ruleid.length===14){
						...
						fract.vinkel.fix();
						fract.vinkel.update((ruleid.charAt(1) + ruleid.charAt(2) + ruleid.charAt(3) + ruleid.charAt(4))/5);
						...
						fract.vinkel.fix();
						fract.vinkel.update((ruleid.charAt(1)+ruleid.charAt(2)+ruleid.charAt(3)+ruleid.charAt(4))/5);
						fract.skewvinkel.update((ruleid.charAt(6)+ruleid.charAt(7)+ruleid.charAt(8)+ruleid.charAt(9))/5);
						fract.al.render();
					}
				}
			}
	...
	module.al =(function(){
		...
		var prepare = function prepare(){
			...
			rads= fract.vinkel.step*5 * (Math.PI / 180);
			...
			var skewrad=(fract.vinkel.step-fract.skewvinkel.step+fract.skewvinkel.steps/2)*5*(Math.PI/180);
		...
		module.render = function render(){
			prepare();
			if (!fract.animate.on && (!fract.vinkel.held && !fract.skewvinkel.held)){
				fract.hash.save();
			}
			i=0;
