//denna håller all information för bakgrunden och måtten.
...
//om man ritar kollar denna funktion vart positionen är.
...
//den här funktionen kollar var den senaste cirkeln man ritade. vilken position.
...
//denna funktion gör så att om du klickar för att rita så börjar den ritandet där din muspekare är. Johan 19b hjälpte mig med denna funktion. Den gör också å att den drar en linje från den sensate cirkeln.
...
//en mouse over-funktion som är att när du håller muspekaren över en färg ändras din nuvarande färg till den du håller över. 
//Den håller i vilket område man kan bytta färg och till vilken färg.
...
//när man ritar är det denna funktion som kollar vilken färg och ser var man ska börja rita.
function myCircle(x, y, z, r, t, color)
{
   ...
   context.fillStyle = color;
   context.fill(100);
//detta är en funktion som gör så att man kan göra kvadrater med färger.
}
...
//Man ställer in hur upplägget för texten ska vara och vilken ordning samt typsnitt
...
// Denna funktion visar färgerna och texten.
...
//rektangel som är blå längst upp
this.myRectangle(x0+-50, y0 + -1* distance, 1264, 40, "blue");
//det är texten där uppe så att man ser vad man ska göra.
...
function check(e) {
    var code = e.keyCode 
// ändra färg och vilka färger som finns samt vilka knappar som aktiverar de
    if (code == 82)
        color = "red"; //ändrar färg
    if (code == 79)
        color = "orange";
    if (code == 89)
        color = "gold";
    ...
    if (code == 69)
        color = "white";
// ändra radie beroende på vilket nummer du klickar på tangentbordet.
if (code == 49)
    radie = 1;
    fill = true;
if (code == 50)
...
if (code == 56)
    radie = 4000;
    fill = true;
//gör så att man reloadar sidan och tar bort allt man har gjort.
if (code == 13)
      window.location.reload();

//gör så att man kan konvertera Celcius till Fahrenheit
}