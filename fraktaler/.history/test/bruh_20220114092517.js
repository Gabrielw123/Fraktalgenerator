...
spara.addEventListener("click", function () {
  ...
  if (rules1.length >= 2) {
    console.log("Du får bara ha ett tecken före (=) i RULES");
  }
  if (!stringrules.includes("=")) {
    console.log("Du måste ha ett (=) i RULES");
  }
  if (!axiom2.includes(rules1)) {
    console.log("Ditt tecken i RULES före (=) har inte något tecken som förekommer i AXIOMEN ");
  }
  confirm("Detta är din input\n"
    "Axiom: " + axiom2 + "\nRules in: " + rules1 + "\nRules out: " + rules2
  );
  axiom = axiom2;
  rules = [
